...
def graphing(i, loop_time, start_time, x_graph, y1_graph, y2_graph, ax2, K, U):
    """Graphing plot specifications

    Args: i (int): iteration
          loop_time (int): loop time (ms)
          start_time (float): start time to find elapsed time later
          x_graph (list): x-axis values for time (s)
          y1_graph (list): y-axis values for kinetic energy (J)
          y2_graph (list): y-axis values for potential energy (J)
          ax2 = right subplot
          K (list): Kinetic Energy (J)
          U (list): Potential Energy (J)

    Output: energy graph subplot
    """
