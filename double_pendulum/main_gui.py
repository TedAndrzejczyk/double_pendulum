...
def create_parameter(col, sub, lower, upper, parameter, num_type):
    ...
    Args: cols (int): slider column
          sub (str): slider subtitle
          lower (int or float): lower slider bound
          upper (int or float): upper slider bound
          parameter (float): specified parameter value
          num_type (int or float): determines what the slider increments by
...
def make_labels(name, value):
    """Makes labels on grid for name and value of gravity ref

    Args: name (str): name for gravity reference
          value (float): gravity value for reference

    Returns: label: label for name on grid
             label: label for value on grid
    """
...
def grid_label(label, num_label, padx, pady):
    """Places the gravity reference labels on the grid along the slider

    Args: label (label): gravity reference label
          num_label (label): gravity reference value
          padx (int): how much separation the values have
          pady (int): how far down on the slider the labels are
    """
...
def create_button(name, col, cmd):
    """Creates a button at the bottom of the gui that the user can press

    Args: name (str): name of button
          col (int): column the button is in on the grid (always at the bottom)
          cmd (lambda expression): command that the button should have when pressed

    Output (if any): whatever "cmd" says
    """
...
def get_sliders(sliders):
    """Find the slider values from sliders

    Args: sliders (list): list of sliders

    Returns: list: list of slider values
    """
...
def main(run, parameters):
    """
    Creates and places sliders, buttons, gravity reference labels
    Can call the physics module which in turn allows for outputs

    Args: run (bool): whether the physics module has to be called
          parameters (list): parameters either default or specified by user
    """
